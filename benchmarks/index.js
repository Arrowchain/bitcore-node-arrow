...
var bitcoind = require('../').services.Bitcoin({
  node: {
    datadir: process.env.HOME + '/.arrow',
    network: {
      name: 'testnet'
    }
  }
...
bitcoind.on('ready', function() {
  ...
  async.series([
    function(next) {
      ...
      suite.add('arrowd getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('arrowd getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('arrowd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('arrowd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
